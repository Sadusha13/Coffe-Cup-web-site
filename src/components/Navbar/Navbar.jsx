      ...
      <div className="container mx-5 px-4 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <img src={Logo} alt="Coffee Cafe logo" className="h-14 w-14 object-contain sm:h-12 sm:w-16" />
            <span className="text-3xl font-bold font-cursive leading-none text-white">Master Cafe</span>
          </div>
          ...
          <div data-aos="fade-down" data-aos-delay="300" data-aos-once="true" className="hidden sm:flex items-center gap-2 md:gap-4">
            <ul className="flex items-center gap-1 md:gap-4 text-white font-medium">
              {menus.map((link) => (
                <li key={link.id}>
                  <a href={link.link} className="inline-block px-2 md:px-3 py-2 text-base md:text-lg text-white/80 hover:text-white duration-200">
                    {link.name}
                  </a>
                </li>
              ))}
        ...
        {isMenuOpen && (
          <div className="sm:hidden pt-2 pb-3">
            <ul className="flex flex-col gap-2 text-white font-medium">
              {menus.map((link) => (
                <li key={link.id}>
                  <a href={link.link} onClick={() => setIsMenuOpen(false)} className="block rounded px-3 py-2 text-lg text-white/85 hover:text-white hover:bg-white/10">
                    {link.name}
                  </a>
                </li>
              ))}
